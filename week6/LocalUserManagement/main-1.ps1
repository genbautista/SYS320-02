...
while($operation){
    ...
    elseif($choice -eq 9){
        ...
        Write-Host "Users with more than 10 failed login attempts in the last $days days:" | Out-String $failedLoginCount | Select-Object Name, Count | Format-Table | Out-String | Write-Host }
    }elseif(-not $choice -eq "[0-10]") {
    Write-Host "Please use numbers from the prompt."
    operation = $true
    }
