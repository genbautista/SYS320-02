...
# Example input: SEC
# Example output: See the screenshots in canvas
function displayCoursesAvailability() {
echo ""
echo -n  "Please Input a Subject Name: "
read courseCode

echo ""
echo "Courses with availability for $courseCode: "
cat "$courseFile" | grep "$courseCode" | awk '$4 > 0' | cut -d';' -f1,2,3,5,6,7,8 | \
sed 's/;/ | /g'
echo ""
}
...
do
	...
	if [[ "$userInput" == "5" ]]; then
		...
		displayCoursesAvailability

	# TODO - 3 Display a message, if an invalid input is given
	else
		echo "Try again. Please choose from options 1-5." 
	fi
done
