...
function displayOnlyPages(){
cat "$logFile" | awk '{print $7}' | grep -E "^/" | sort | uniq -c
}
...
if [[ ! -f "${iocFile}" ]]
then 
echo "No indicator attacks"
return 
fi
egrep -i -f "$iocFile" "$logFile" | awk '{print $1}' | sort | uniq -c
}
...
do
	...
	echo "[3] Display only visited pages"
	echo "[4] Histogram"
	...
	# Display a message, if an invalid input is given
		...
		echo "Why did you input something outside of 1-7? Try again."
	fi
done
